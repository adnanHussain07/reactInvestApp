...
function WithdrawApproveHeader(props) {
  ...
  const withdrawApprovePagination = useSelector(({ auth }) => auth.sharedData.withdrawApprovePagination);
  ...
  React.useEffect(() => {
    let mounted = true;
    if (mounted) {
      if (withdrawApprovePagination && !isEmptyObject(withdrawApprovePagination) && withdrawApprovePagination.pageNo && withdrawApprovePagination.pageSize) {
        callGo();
      }
    }
